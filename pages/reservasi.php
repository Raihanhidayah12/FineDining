...
// Ambil semua area (tersedia dan tidak tersedia)
$areas = [];
...
// Ambil semua menu (tersedia dan tidak tersedia) untuk customer
$menus = [];
            ...
            <div class="card p-4">
                <div class="mb-3">
                    ...
                    <select class="form-select" id="seatingSelect" aria-label="Select Seating Area" onchange="updateSeatingImageAndCapacity()">
                        <option value="" style="color: #6c757d;">Select Seating</option>
                ...
                <div id="seatingImage" class="mt-4 text-center" style="display: <?php echo $nama_area ? 'block' : 'none'; ?>;">
                    <img src="<?php echo isset($areas[$nama_area]['gambar_area']) ? $areas[$nama_area]['gambar_area'] : $fallback_image; ?>" class="card-img-top" alt="Selected Seating" id="seatingAreaImage">
                </div>
            </div>
        </div>
                ...
                <div class="mt-4 mb-4">
                    ...
                    <div class="custom-date-time">
                        <select class="form-select" id="customTime" aria-label="Select Time">
                            ...
                            <?php
                            $times = ['10:00 AM', '11:00 AM', '12:00 PM', '01:00 PM', '02:00 PM', '03:00 PM', '04:00 PM', '05:00 PM', '06:00 PM', '07:00 PM', '08:00 PM', '09:00 PM', '10:00 PM'];
                    ...
                    <small class="text-muted">Available between 10:00 AM and 10:00 PM</small>
                </div>
            </div>
        </div>
                ...
                <?php foreach ($categories as $category): ?>
                    <div class="col-12" data-aos="zoom-in" data-aos-delay="<?php echo (array_search($category, $categories) + 1) * 100; ?>">
                        <div class="card p-4 card-menu">
                            <h5 class="category-title"><?php echo ucfirst(str_replace(['makanan', 'minuman'], ['Food', 'Drink'], htmlspecialchars($category))); ?></h5>
                            <div class="menu-list">
                                <?php if (isset($menus[$category]) && !empty($menus[$category])): ?>
                                    ...
                                    <?php endforeach; ?>
                                <?php else: ?>
                                    <p class="text-muted text-center">No <?php echo ucfirst(str_replace(['makanan', 'minuman'], ['Makanan', 'Minuman'], htmlspecialchars($category))); ?> available</p>
                                <?php endif; ?>
                            </div>
                        </div>
                    </div>
...
<div id="customAlert" class="custom-alert">
    <p id="alertMessage">Please select your preferred seating, table number, date, time, and at least one available menu item to proceed.</p>
...
<script>
    ...
    try {
        ...
        console.error('Pikaday failed to initialize:', e);
    ...
    function updateSeatingImageAndCapacity() {
        ...
        if (selectedOption.value) {
            ...
            console.log('Attempting to load seating area image: ' + imageSrc);
            imgElement.src = imageSrc;
            imgElement.onerror = function() {
                console.error('Failed to load seating area image: ' + imageSrc);
                imgElement.src = '<?php echo $fallback_image; ?>';
            };
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        menuImages.forEach(img => {
            img.onerror = function() {
                console.error('Failed to load menu image: ' + img.src);
    ...
    function redirectToInterior() {
        ...
        if (!seatingSelect || !tableSelect || !capacityDisplay || !dateInput || !timeSelect) {
            showAlert('An unexpected error has occurred. Please refresh the page.');
            return;
        }
        ...
        if (!seating || !tableNumber || !capacity || capacityText === 'Please select a table' || capacityText.includes('Error') || !date || !time) {
            showAlert('Please select your preferred seating, table number, date, and time to proceed.');
            return;
        }
        ...
        const timeValue = time.match(/(\d{2}):(\d{2})\s?(AM|PM)?/i);
        if (timeValue) {
            let hours = parseInt(timeValue[1]);
            if (timeValue[3] && timeValue[3].toUpperCase() === 'PM' && hours !== 12) hours += 12;
            else if (timeValue[3] && timeValue[3].toUpperCase() === 'AM' && hours === 12) hours = 0;
            if (hours < 10 || hours > 22) {
                showAlert('Please choose a time between 10:00 AM and 10:00 PM.');
                return;
            }
        } else {
    ...
    function updateTimeOptions() {
        ...
        const availableTimes = ['10:00 AM', '11:00 AM', '12:00 PM', '01:00 PM', '02:00 PM', '03:00 PM', '04:00 PM', '05:00 PM', '06:00 PM', '07:00 PM', '08:00 PM', '09:00 PM', '10:00 PM'];
        ...
        if (selectedDate < todayDate) {
            ...
            if (currentTime >= 22) {
                ...
                picker.setMinDate(new Date(moment().add(1, 'days').format('YYYY-MM-DD')));
            } else {
                availableTimes.forEach(time => {
                    const hour = parseInt(time.match(/(\d{2}):/)[1]);
                    const period = time.match(/(AM|PM)/)[0];
                    const currentHour = currentTime % 12 || 12;
                    const currentPeriod = currentTime >= 12 ? 'PM' : 'AM';
                    if (period === currentPeriod && hour < currentHour) {
                        timeSelect.innerHTML += `<option value="${time}" disabled>${time} (Not available)</option>`;
                    } else {
    ...
    function submitReservation() {
        ...
        if (!seating || !tableNumber || !capacity || capacityText === 'Please select a table' || capacityText.includes('Error') || !date || !time || menuItems.length === 0) {
            showAlert('Please select your preferred seating, table number, date, time, and at least one available menu item to proceed.');
            return;
        }
