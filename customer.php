...
/* Hide vertical scrollbar but keep scrollable */
html, body {
...
.card-title, .card-text, .card-text.text-muted {
  color: var(--text-light) !important;
}
...
.menu-item:hover {
  box-shadow: 0 20px 40px rgba(212, 175, 55, 0.4), 0 0 15px var(--primary-gold);
...
.carousel-btn:focus {
  outline: 2px solid выраждуvar(--primary-gold);
  outline-offset: 2px;
}
...
.how-to-reserve-card:hover {
  transform: translateY(-5px);
}

.how-to-reserve-card .step-number {
  font-size: 2.5rem;
  color: var(--primary-gold);
  font-weight: 900;
  margin-bottom: 1rem;
...
.how-to-reserve-card h5 {
  color: var(--text-light);
  font-size: 1.5rem;
  margin-bottom: 1rem;
}

.how-to-reserve-card p {
  color: var(--text-light);
  opacity: 0.9;
  font-size: 1rem;
}
...
// Start session to store selected table and area
...
<header class="hero-bg-unified text-light position-relative" data-aos="fade-down">
  <div class="container">
    ...
    if (isset($_SESSION['user']) && !empty($_SESSION['user'])) {
      ...
      echo '<h1 class="fw-bold display-4 mb-4">Welcome, ' . $username . '!</h1>';
      echo '<p class="lead mb-5">Savor an unforgettable dining experience tailored just for you.</p>';
    } else {
      echo '<h1 class="fw-bold display-4 mb-4">Experience the Best Culinary Journey</h1>';
      echo '<p class="lead mb-5">Discover special menus and a cozy ambiance for your memorable moments.</p>';
    }
    ?>
    <div class="d-flex justify-content-center gap-3 flex-wrap">
      <a href="../pages/reservasi.php" class="btn btn-primary shadow">Reserve Your Table</a>
...
<section class="container py-5" data-aos="fade-up" id="seating-experiences">
  ...
  <div class="row g-4">
    ...
    foreach ($areas as $nama_area => $area) {
        ...
        $gambar_area = str_replace('http://', 'https://', $gambar_area);
...
<section class="py-5 bg-dark" id="menu" data-aos="fade-up">
  ...
  <div class="container position-relative">
    ...
    <div class="text-center mt-5" data-aos="fade-up" data-aos-delay="200">
      <a href="pages/menu.php" class="btn btn-outline-light rounded-pill px-5">View Full Menu</a>
    </div>
  </div>
</section>
  ...
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="loginModalLabel">Reservation Required</h5>
        <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
      <div class="modal-body">
        <p>Please make a reservation first to proceed with your order and enjoy our exclusive dining experience.</p>
      </div>
      <div class="modal-footer">
        <button type="button" class="btn btn-secondary rounded-pill" data-bs-dismiss="modal">Close</button>
        <a href="../pages/reservasi.php" class="btn btn-primary rounded-pill">Reserve Now</a>
      </div>
    </div>
  </div>
</div>
...
<div id="customToast" class="custom-toast" role="alert">
  <span id="toastMessage">Please select a table number before reserving.</span>
</div>
  ...
  <div class="row g-4">
    ...
    foreach ($steps as $i => $step) {
      ...
      echo '<div class="col-md-4" data-aos="fade-up" data-aos-delay="' . $delay . '">
        <div class="how-to-reserve-card h-100">
...
// Scroll menu carousel left or right
...
// Keyboard accessibility for carousel buttons
...
// Initialize Particles.js for background effect (only on larger screens)
...
// Handle table selection for reservation
function selectTableForReservation(button, nomorMeja, namaArea) {
  ...
  sessionStorage.setItem('selected_nama_area', namaArea);
  sessionStorage.setItem('selected_nomor_meja', nomorMeja);
}

// Handle reserve button click
function reserveTable(event, namaArea) {
  event.preventDefault();
  const selectedNomorMeja = document.getElementById('selected_nomor_meja_' + namaArea).value;

  // Show toast if no table is selected
  if (!selectedNomorMeja) {
    const toast = document.getElementById('customToast');
    const toastMessage = document.getElementById('toastMessage');
    toastMessage.textContent = 'Please select a table number before reserving.';
  ...
  // Animate button and redirect to reservation page
  const reserveBtn = event.target;
  gsap.fromTo(reserveBtn, { scale: 1 }, {
    scale: 1.1,
    duration: 0.5,
    ease: "power2.out",
    onComplete: () => {
      window.location.href = `./pages/reservasi.php?nama_area=${encodeURIComponent(namaArea)}&nomor_meja=${selectedNomorMeja}`;
    }
  });
}
</script>